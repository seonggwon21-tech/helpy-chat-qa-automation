...
class TestLnbChatHistory:
    ...
    def test_previous_chat_preserved_in_lnb(self, logged_in_driver):
        ...
        # 3. [검증 1] LNB 목록에 기존 대화 항목 존재 확인 (명시적 대기)
        chat_page.wait.until(EC.presence_of_element_located(self.LNB_CHAT_ITEMS))
        lnb_items = logged_in_driver.find_elements(*self.LNB_CHAT_ITEMS)
        assert len(lnb_items) > 0, "LNB 목록에 기존 대화가 존재하지 않습니다."
        logger.info(f"LNB 목록 확인 완료 - 대화 항목 {len(lnb_items)}개 존재")

        # 4. 첫 번째 대화 클릭 (명시적 대기)
        chat_page.wait.until(EC.element_to_be_clickable(self.LNB_CHAT_ITEMS)).click()
        logger.info("LNB 첫 번째 대화 클릭 완료")

        # 5. [검증 2] 클릭 후 AI 응답 내용 유지 확인 — 대화 로드 완료까지 대기
