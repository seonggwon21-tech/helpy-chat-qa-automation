"""
커뮤니티 API 검증
포함 TC: TC_012 (에이전트 목록 조회), TC_013 (에이전트 수 조회),
         TC_014 (에이전트 상세 조회), TC_015 (미인증 접근 거부)
"""
...
@allure.story("에이전트 목록·수·상세 조회 시나리오")
class TestAgentAPI:

    @allure.title("에이전트 목록 조회 → 수 확인 → 상세 조회 시나리오")
    def test_agent_read_flow(self, api_session, api_base_url):
        """에이전트 목록(TC_012) → 수 조회(TC_013) → 상세 조회(TC_014) 전체 플로우"""

        with allure.step("[TC_012] GET /agent → 200 및 리스트 구조 확인"):
        ...
        with allure.step("[TC_013] GET /agent/count → 200 및 count 정수 반환 확인"):
        ...
        with allure.step("[TC_014] GET /agent/{agent_id} → 200 및 상세 데이터 구조 확인"):
            agent_id = agents[0].get("id") or agents[0].get("agent_id")
            assert agent_id, "TC_012 응답에서 agent_id를 가져올 수 없습니다."
...
class TestChatroomAuth:
    ...
    def test_unauthorized_returns_401_or_403(self, api_base_url):
        with allure.step("[TC_015] 인증 없이 GET /chatroom 요청"):
